...
# Define training protocol for gCNN
def train():
    with wandb.init() as run:
        config = run.config
        
        ph_data = np.array(np.load("file/path", allow_pickle=True), dtype=np.float64)
        img_data = np.array(np.load("file/path", allow_pickle=True), dtype=np.float64)
        lb = np.array(np.load("file/path", allow_pickle=True), dtype=np.float64)
        ...
        CNN_model = Model(inputs=[image_input, ph_input], outputs=[output_layer])
